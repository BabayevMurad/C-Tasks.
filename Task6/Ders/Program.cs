...
class Debtor
{
    ...
    {
        if (this.Debt == other?.Debt) return 0;
        if (this.Debt.CompareTo(other?.Debt) == 1) return -1;
        return 1;
...
class Program
{
    ...
    public static void Main()
    {
        ...
        //var result = debtors.Where(deb => 1998 > deb.BirthDay.Year && deb.BirthDay.Year > 1988).ToList();

        //foreach (var deb in result)
        //{
        ...
        //var result = debtors.Where(deb => deb.Debt < 5000).Select(deb => $"{deb.FullName} Borc:{deb.Debt}").ToList();
        ...
        //var result = debtors.Where(deb => deb.FullName.Length > 18).ToList();
        ...
        //{
        //    int count = 0;
        //    var arry = item.Phone.ToCharArray();
        //    foreach (var item2 in arry)
        //    {
        //        if (item2 == '7') {
        //            count++;
        //        }
        ...
        //var result = debtors.Where(deb => deb.BirthDay.Month == 12 || deb.BirthDay.Month < 3 ).ToList();
        ...
        //double ortaborc = umumiborc / debtors.Count;

        //var result = debtors.Where(deb => deb.Debt > ortaborc);
        ...
        //var selected = debtor.Select(deb => $"{deb.FullName} Borc: {deb.Debt}").ToList();

        //foreach (var deb in selected)
        //{
        //Console.WriteLine(deb);
        //}
        ...
        //foreach (var item in debtors)
        //{
        //    var ary = item.FullName.ToCharArray();


        //    int count = default;
        //    foreach (var item3 in ary)
        //    {
        //        count = 0;
        //        var selecteditem = item3;
        //        foreach (var item2 in ary)
        //        {
        ...
        //foreach (var item in debtors)
        //{
        //    if (result.Debt < item.Debt)
        //    {
        //        result = item;
        //    }
        //}

        //Console.WriteLine(result.BirthDay.Year);
        ...
        //Console.WriteLine(umuniborc);

        //    11)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        //var result = debtors.Where(deb => deb.BirthDay.Year <= 1945);
        ...
        //foreach (var debtor1 in debtors)
        //{
        //    var arry = debtor1.Phone.ToCharArray();
        ...
        //foreach (var debtor1 in debtors)
        //{
        //    if (debtor1.BirthDay.Month < DateTime.Now.Month)
        //    {
        //        var result = DateTime.Now.Month - debtor1.BirthDay.Month;

        //        if (result*500 >= debtor1.Debt)
        //        {
        //            debtor.Add(debtor1);
        //        }
        //    }
        //    else if (debtor1.BirthDay.Month > DateTime.Now.Month)
        //    {
        //        var result = debtor1.BirthDay.Month - DateTime.Now.Month;
        //    }
        ...
        {
            int count = 0;
            var arry = item.FullName.ToCharArray();
            var arry1 = new char[] { 's', 'm', 'i', 'l', 'e' };
            foreach (var item2 in arry1)
            {
                int count1 = 0;
                foreach (var item3 in arry)
                {
